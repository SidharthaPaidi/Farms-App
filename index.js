...
app.get('/products', async (req, res) => {
    ...
    const { id } = req.params;
    const farm = await Product.findById(id).populate('farm', 'name','city','email');
    if (category) {
        const products = await Product.find({ category })
        res.render('products/index', { products, category ,farm})
    } else {
        const products = await Product.find({})
        res.render('products/index', { products, category: 'All',farm })
    }
})
